...
summary = 'ROI scan as in Report 29 for the final version of model and data'
gpu_id = '0'
...
for sim_seed in tqdm(sim_seeds, position = 0):
    ...
    for outer_radius in tqdm(outer_radius_list, position=1, leave=False):
        ...
        if os.path.isfile(fit_dir + file_name):
            svi_seed += 1
            str_svi_seed = str(svi_seed)
            print('sim_seed:', sim_seed, 'svi_seed:', svi_seed)
            continue   
        # else: # for identifying incomplete jobs
        #     svi_seed += 1
        #     str_svi_seed = str(svi_seed)
        #     print('sim_seed:', sim_seed, 'svi_seed:', svi_seed)
        #     continue
