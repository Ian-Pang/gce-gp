...
def make_corner_plots(samples, with_mean_vlines = False, sim_vlines = False, temp_dict = None, print_latex_means = False, labels = None, fig = None):
    '''
    Make corner plots from samples dictionary
    ...
    if fig == None:
        exists_fig = False
        fig = corner.corner(template_sample_array.T, labels=labels, quantiles=[0.16, 0.5, 0.84], show_titles=True, title_kwargs={"fontsize": 12})
    else:
        exists_fig = True
        corner.corner(template_sample_array.T, labels=labels, quantiles=[0.16, 0.5, 0.84], show_titles=True, title_kwargs={"fontsize": 12}, fig=fig)
...
def gen_labels(keys, num_theta = None):
    def format_S(k):
        if 'S' in k:
    ...
    for k in keys:
        if 'theta' in k:
            for i in range(num_theta):
                labels.append(format_theta(k, gen_blg_name_(i)[0]))
        else:
...
def generate_temp_sample_maps(samples, ebinmodel, gp_samples = None, custom_num = None, nfw_gamma = 'vary', custom_mask = None):
...
def generate_temp_sample_maps_theta(samples, ebinmodel, gp_samples = None, custom_num = None, nfw_gamma = 'vary', custom_mask = None):
    # NOTE: Does not create samples for disk or Ylm
    ...
    for name in ordered_temp_names:
        ...
        if name in ['iso', 'psc', 'bub']: # rigid templates
            ...
            S_ics = samples['S_' + name]
            ics_temps_at_bin = jnp.asarray([ics_temp.at_bin(ie, mask=mask_p) for ics_temp in ebinmodel.ics_temps])
            if ('theta_' + name)  not in list(samples.keys()):
                temp_sample_dict[name] = np.array([S_ics[i] * ics_temps_at_bin[0] for i in tqdm(range(num_samples))])
            else:
                theta = samples['theta_' + name]
                ics_temp = np.dot(theta, ics_temps_at_bin)
                temp_sample_dict[name] = np.array([S_ics[i] * ics_temp[i,:] for i in tqdm(range(num_samples))])
        elif name == 'blg':
            S_blg = samples['S_' + name]
            blg_temps_at_bin = jnp.asarray([blg_temp.at_bin(ie, mask=mask_p) for blg_temp in ebinmodel.blg_temps])
            if ('theta_' + name)  not in list(samples.keys()):
                ...
                temp_sample_dict[name] = np.array([S_blg[i] * blg_temp[i,:] for i in tqdm(range(num_samples))])
...
def tot_log_counts_hist(temp_sample_dict, temp_sim_dict = None, temp_sim_names = None, mask = None, bins = np.linspace(1,5,100), alpha = 0.75, histtype = 'step', gp_model_nfw = False, gp_model_iso = False, 
                        ax = None, display_x_info = True, display_y_info = True, legend = True):
    if ax is None:
    ...
    if mask is not None and temp_sim_dict is not None:
        temp_sim_dict = {name: temp_sim_dict[name][~mask] for name in temp_sim_dict.keys() if name in all_temp_names}
    ...
    # gce_sum = (temp_sample_dict['blg'] + temp_sample_dict['nfw']).sum(axis = 1)
    # ax.hist(np.log10(gce_sum), bins = bins, alpha = alpha, label = 'nfw+blg', density = True, histtype = histtype, color = 'k')
    ...
    ax.legend(frameon = False) if legend else None
    ax.set_xlabel(r'$\log_{10}(\mathrm{Counts})$') if display_x_info else None
    ax.set_ylabel(r'$\mathrm{Density}$') if display_y_info else None
    if ax is None:
        return fig
...
def healpix_to_cart(map_data, mask_roi, n_pixels=80, nside = 128, map_size = 40, nan_fill = False):
    if nan_fill:
        map_pred = np.zeros(hp.nside2npix(nside)) * np.nan
    else:
...
def multi_healpix_to_cart(map_data_samples, mask_roi, n_pixels=80, nside = 128, nan_fill = False, progress_bar=True):
    num_samples = map_data_samples.shape[0]
    map_data_samples_cart = np.zeros((num_samples, n_pixels, n_pixels))
    if progress_bar:
        for i in tqdm(range(num_samples)):
            map_data_samples_cart[i] = healpix_to_cart(map_data_samples[i], mask_roi, n_pixels = n_pixels, nside = nside, nan_fill=nan_fill)
    else:
...
def load_main_dir():
    return '/data/edr76/gce-gp/'

def load_data_dir(sim_name):
    main_dir = load_main_dir()
...
from numpyro.distributions.transforms import ExpTransform, AffineTransform, ComposeTransform
# looks like it works!
def ExpUniform(low, high):
    '''
    Returns a distribution that is uniform in [low, high] after applying exp transform
    Requires to be standardized if low is negative
    Verified that the log_prob is correct
    ...
    Returns:
    - dist.TransformedDistribution: a distribution that is uniform in [low, high] after applying exp transform
    
    - Note: The distribution is useful for defining priors on scale parameters
            But, we can use it to conveniently transform from to and from physical normalizations
    '''
    uni_dist = dist.Uniform(low=low, high=high)
    range = high - low
    loc = - low / range
    scale = 1. / range
    std_transform = AffineTransform(loc = loc, scale = scale)
    transform = ComposeTransform([std_transform, ExpTransform().inv])
    return dist.TransformedDistribution(uni_dist, transform)

# integer to roman numeral converter for Ilias model processing
...
def ilias_models():
    '''
    # want to associate model with a number (1-80)
# for combined model fit, we would just need to scan over all the template names and add them to the model
# taken from new_dataset/process_ed_top_all.ipynb

    '''
    # ordered using numbering convention from 2112.09706 fig. 12
    N = 0
    number_to_letter = {}
    with open('../data/ilias_zenodo/ZENODO_FILES/NAMING_CONVENTION_OF_DIFFUSE_EMISSION_MODELS.dat') as f:
        for line in f:
            parts = line.split()
            number_to_letter[parts[0]] = parts[1]
            N += 1

    mod_list = []
    roman_numerals = [int_to_Roman(i) for i in range(1,N)]
    for name in roman_numerals:
        letter = number_to_letter[name]
        mod_list.append(letter)

    return mod_list

def generate_ll_samples(temp_sample_dict, data):
    
    N_samples = temp_sample_dict['pib'].shape[0]
    N_data = data.shape[0]
    temp_sim = jnp.zeros((N_samples, N_data))
