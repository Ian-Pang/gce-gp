...
# from numpyro.infer.elbo import Trace_ELBO_2 # necessary for rate optimization approach (see https://github.com/edwarddramirez/svi-dist-fit/02_kl_nonparam.ipynb for more details)
...
class EbinPoissonModel:
    ...
    def __init__(
        ...
        self.norm_mask_r_outer = norm_mask_r_outer
        ...
        if self.nside > 128:
            ebin_data_dir = main_dir + '/data/fermi_data_573w/ebin'
            if not os.path.isdir(ebin_data_dir):
                print('NSIDE > 128 requires ebin_512 dataset.')
        else:
            ebin_data_dir = main_dir + '/data/fermi_data_573w/ebin_128'
        default_data_dir = main_dir + '/data/fermi_data_573w/fermi_data_256'
        ...
        if data_file == 'fermi_data':
            ...
            counts_sum = counts[10:20].sum(axis=0)
            self.counts = jnp.array(               # artificially make 40 energy bins
                np.tile(counts_sum, (40, 1))
            )
        elif data_file == 'custom':
            print('Manually Load Data File in Notebook!')
        else: 
            # sim_data_dir = main_dir + '/data/synthetic_data/' + data_file + '/'
        ...
        if not self.no_ps_mask:
            ...
            # mask_ps_arr_no_eng = np.load(main_dir + '/data/mask_3fgl_0p8deg.npy') # shape (196608,): no dependence on energy bins ; already Boolean so no inequality
            # NOTE: Replace 0.8 deg mask with 3FGL masking from GCE_NN repo (2-20 GeV update)
            # TODO: Add option to select between {0.8 deg, 3fgl, 4fgl} masking
            mask_ps_arr_no_eng = np.array(
                np.load(main_dir + 'data/masks/4FGL_DR2_mask_cl=0.900000.npy'),
                dtype = np.bool_
             ) # shape (196608,): no dependence on energy bins ; manually make Boolean
        ...
        self.normalization_mask = np.asarray(
            cm.make_mask_total(
                ...
                mask_ring=True,
                inner=0,
                outer=self.norm_mask_r_outer,
            )
        )

        # # use all other pixels as data
        # pre_outer_mask = np.asarray([
        #     cm.make_mask_total(
        #         nside=128,
        #         band_mask=False,
        ...
        #     )
        # ])
        # pre_outer_mask = ~pre_outer_mask + mask_ps_arr_no_eng
        # pre_outer_mask_arr = np.tile(pre_outer_mask, (40, 1))

        # # simple outer mask
        # self.outer_mask = np.asarray([
        #     cm.make_mask_total(
        #         nside=self.nside,
        #         band_mask=True,
        #         band_mask_range=self.mask_roi_b,
        #         mask_ring=False,
        #         custom_mask=pre_outer_mask_eng
        #     )
        #     for pre_outer_mask_eng in pre_outer_mask_arr
        # ])
        self.outer_mask = np.asarray([
            cm.make_mask_total(
                nside=self.nside,
    ...
    def load_templates(self, temp_list, blg_names, dif_names, ie = 10):
        ...
        if 'psc' in temp_list:
            ...
            #     norm_mask=self.normalization_mask,
            # )
            # NOTE: Replace Yitian's 3fgl energy-dependent psf with 3fgl energy-independent psf (2-20 GeV)
            self.temps['psc'] = EbinTemplate(
                to_nside(np.load(f'{ebin_data_dir}/tracy-psc-bestpsf-4fgl-dr2.npy'), self.nside),
                norm_mask=self.normalization_mask,
            )
        if 'bub' in temp_list:
            ...
            # updated bubbles template 
            # located in fermi_data_573w/fermi_data_256 for consistency with previous 
            # but actually nside = 512 maps
            # options: 1407.7905, 1704.03910, 1901.03822, concatenated version of latter two
            self.temps['bub'] = Template(
                to_nside(np.load(f'{default_data_dir}/bubble_concat_out_hp.npy'), self.nside),
                norm_mask=self.normalization_mask
            )
        if 'dsk' in temp_list:
        ...
        #     )
        if 'nfw' in temp_list:
            self.nfw_temp = NFWTemplate(nside=self.nside, custom_mask = self.normalization_mask)
            self.nfw_temp.set_mask(self.mask_roi_arr[ie])
            if self.nfw_gamma != 'vary':
                gamma = self.nfw_gamma
                self.nfw_temp_fixed = self.nfw_temp.get_NFW2_template(gamma=gamma)
        ...
        # load blg templates with gtsrcmap applied
        if ('blg' in temp_list) or (self.u_weights == 'blg'):
            self.n_blg_temps = len(self.blg_names)
            self.blg_temps = [
                Template(
                    to_nside(np.load(f'{default_data_dir}/{blg_name}.npy'), self.nside),
                    norm_mask=self.normalization_mask,
                )
                for blg_name in blg_names
            ]
        ...
        if before_fit == True:
            ...
            if scale == 'float':
                if self.gp_scale_option == 'Linear':
                    ...
                    scale_off = scale[2:]
            if amp == 'float':
                amp = numpyro.param('amp', 3. * jnp.ones(()), constraint=dist.constraints.positive)
    ...
    def config_model(self, ebin=10):
        ...
        if self.is_gp:
            ...
            if self.u_option == 'None':
                ...
                # made using ../scratch/grid.ipynb in gce-prob-prog-ed-v0.2 main directory
                    ...
                    N, Nc = np.load(main_dir + '/utils/Ncirc_vs_Nsq.npy', allow_pickle = True)
                    ...
                    N_arr, d_arr = np.load(main_dir + '/utils/Nhex_vs_d.npy', allow_pickle = True)
        ...
        # generate ExpUniform distribution for S_priors
        faint_temps = ['iso', 'psc', 'bub', 'blg']
        bright_temps = ['pib', 'ics', 'nfw', 'dsk']
        faint_ExpUniform = ef.ExpUniform(low = 1e-3, high = 50)
        bright_ExpUniform = ef.ExpUniform(low = 1e-3, high = 100.)
        S_prior_dict = {}
        for temp in faint_temps:
            S_prior_dict[temp] = faint_ExpUniform
        for temp in bright_temps:
            S_prior_dict[temp] = bright_ExpUniform
        self.S_prior_dict = S_prior_dict
    ...
    def model(self, ebin=10, gp_rng_key = jax.random.PRNGKey(4242)):
        ...
        if self.n_dif_temps > 0:
            ...
            if self.n_dif_temps > 1:
                if 'pib' in self.temp_list:
                    theta_pib = numpyro.sample("theta_pib", dist.Dirichlet(jnp.ones((self.n_dif_temps,))))
                if 'ics' in self.temp_list:
                    theta_ics = numpyro.sample("theta_ics", dist.Dirichlet(jnp.ones((self.n_dif_temps,))))
...
#                 gamma = self.nfw_gamma
        ...
        if self.is_gp:
            ...
            if self.u_option == 'None':
                ...
                if self.gp_deriv:
                    ...
                    kernel = base_kernel
                    gp_u = GaussianProcess(kernel, xu_f, diag=1e-3, mean = 0.) # p(u)
        ...
        with numpyro.plate('data', size=len(data), dim=-1, subsample_size=self.Nsub) as ind:
            ...
            if 'nfw' in self.temp_list:
                if self.nfw_gamma == 'vary':
                    ...
                    mu += S_nfw * self.nfw_temp_fixed[~mask][ind]
        ...
        # compute log likelihood on outer mask
        # initialize array of counts
        outer_data = self.counts[ie][~outer_mask]
        omu = jnp.zeros(len(outer_data))
        
        #===== rigid templates =====
        # all templates should be already normalized
        if 'iso' in self.temp_list:
            omu += S_iso * jnp.asarray(self.temps['iso'].at_bin(ie, mask=outer_mask))
        if 'psc' in self.temp_list:
            omu += S_psc * jnp.asarray(self.temps['psc'].at_bin(ie, mask=outer_mask))
        if 'bub' in self.temp_list:
            omu += S_bub * jnp.asarray(self.temps['bub'].at_bin(ie, mask=outer_mask))
        ...
        if self.n_dif_temps > 0:
            if self.n_dif_temps == 1:
                if 'pib' in self.temp_list:
                    omu += S_pib * self.pib_temps[0].at_bin(ie, mask=outer_mask)
                if 'ics' in self.temp_list:
                    omu += S_ics * self.ics_temps[0].at_bin(ie, mask=outer_mask)
            else:
                if 'pib' in self.temp_list:
                    pib_temps_at_bin = jnp.asarray([pib_temp.at_bin(ie, mask=outer_mask) for pib_temp in self.pib_temps])
                    omu += S_pib * jnp.dot(theta_pib, pib_temps_at_bin)
                if 'ics' in self.temp_list:
                    ics_temps_at_bin = jnp.asarray([ics_temp.at_bin(ie, mask=outer_mask) for ics_temp in self.ics_temps])
                    omu += S_ics * jnp.dot(theta_ics, ics_temps_at_bin)
            
            if 'blg' in self.temp_list:
                if self.n_blg_temps == 1:
                    omu += S_blg * self.blg_temps[0].at_bin(ie, mask=outer_mask)
                else:
                    blg_temps_at_bin = jnp.asarray([blg_temp.at_bin(ie, mask=outer_mask) for blg_temp in self.blg_temps])
                    omu += S_blg * jnp.dot(theta_blg, blg_temps_at_bin)
            
        #===== variable templates =====
        if 'nfw' in self.temp_list:
            if self.nfw_gamma == 'vary':
                omu += S_nfw * self.nfw_temp.get_NFW2_template(gamma=gamma)[~outer_mask]
            else:
                omu += S_nfw * self.nfw_temp_fixed[~outer_mask]
        
        if 'dsk' in self.temp_list:
            if self.disk_option in ['vary', 'fixed']:
                if self.disk_option == 'vary':
                    temp_dsk = self.dsk_temp.get_template(zs=zs, C=C)[~outer_mask]
                else:
                    temp_dsk = self.temps['dsk'].at_bin(ie, mask=outer_mask)
                omu += S_dsk * temp_dsk
    
        # ===== Poisson likelihood =====
        numpyro.factor('outer_log_likelihood', log_like_poisson(omu, outer_data)) # L(x|u)
    ...
    def cfit_SVI(
        ...
        guide='iaf', optimizer=None, num_flows=3, hidden_dims=[64, 64],
        n_steps=5000, lr=0.006, num_particles=8, progress_bar = True,
        early_stop = np.inf, record_states = True, record_min = True, 
        ...
        **model_static_kwargs,
        ...
        self.svi_results = self.svi_loop(svi, svi_state, progress_bar = progress_bar, num_steps = n_steps, rng_key = rng_key, early_stop = early_stop, 
                                         record_states = record_states, record_min = record_min, **model_static_kwargs)
        self.svi_model_static_kwargs = model_static_kwargs
    ...
    def svi_loop(self, svi, svi_state, progress_bar = True, num_steps = 1000, rng_key = jax.random.PRNGKey(0), early_stop = np.inf,
                   ebin = 10, record_states = True, record_min = True):
        ...
        if progress_bar:
            with tqdm.trange(1, num_steps + 1) as t:
                ...
                for i in t:
                    ...
                    if jnp.isnan(loss):
                        print('svi loop: loss = NAN')
                        break
            losses = jnp.stack(losses) 
        else:
            if (record_states == True) or (record_min == True): 
                ...
                svi_states, losses = lax.scan(body_fn, svi_state, None, length=num_steps)
        ...
        if record_min == False:
            min_params = min_svi_state = min_loss = None
        else:
            min_params = svi.get_params(min_svi_state)
            
        if record_states == False:
            recorded_steps = recorded_params = recorded_states = None
        ...
        def body_fn(params, rng_key):
            ...
            if 'nfw' in self.temp_list:
                if self.nfw_gamma == 'vary':
                    ...
                    mu += temp_samples['S_nfw'][:,None] * self.nfw_temp_fixed[~mask][None,:]
    ...
    def cget_gp_samples_vec(self, rng_key, num_samples, svi_results = None, custom_mask = None):
        # development: v0.2/nfw_recovery/1d_gaussian_example/step_2_sidd_debug.ipynb
        ...
        # allow for custom masking
        if custom_mask is None:
            x_p = x
        else:
            x_p = ef.get_x_from_mask(custom_mask,nside) # predicted x given sampled u
        ...
        def generate_data(gp_rng_key):
            if u_option == 'None':
                ...
                log_rate_u = pred(gp_u_key, ie)['log_rate_u'].T
                log_rate_u = jnp.squeeze(log_rate_u, axis = -1) # shape mismatch otherwise (shape_ll ~ (shape_x, shape_x))
                _, gp_x = gp_u.condition(log_rate_u, x_p, diag=1e-3) # p(x|u)
    ...
    # replacement to cget_gp_samples_vec
    # IMPROVEMENT: generates samples from any template in a way that preserves correlations
    # TODO: Super redundant again, generate_data appears in 3 different functions
    # Would want to clean this code up and remove all the redundancies
    # For now, just pressing alone
    def cget_samples_vec(self, name, rng_key, num_samples, svi_results = None, custom_mask = None):
        # development: second half of gce-prob-prog-ed-v0.3/ilias_fits_to_data/gp_fit_data_nb_1.ipynb
        ...
        def generate_data(gp_rng_key):
            ...
            if (name == 'log_rate'):
                ...
                if u_option == 'None':
                    log_rate = pred(gp_key, ie)['log_rate'].T
                    log_rate = jnp.squeeze(log_rate, axis = -1) # shape mismatch otherwise (shape_ll ~ (shape_x, shape_x))

                    if custom_mask is None:
                        return log_rate
                    else:
                        _, gp_x = gp_u.condition(log_rate, x_p, diag=1e-3) # p(x|u)
                        log_rate_p = gp_x.sample(gp_key)
                        return log_rate_p
                else:
                    xu_f = self.load_inducing_points(before_fit = False, params = params)
                ...
                samples_name = pred(gp_u_key, ie)[name].T
                samples_name = jnp.squeeze(samples_name, axis = -1) # shape mismatch otherwise (shape_ll ~ (shape_x, shape_x))
        ...
        def body_fn():
            gp_rng_key_vec = jax.random.split(rng_key, num_samples)
            samples = generate_data_vec(gp_rng_key_vec)
            return samples
        
        return jit(body_fn)()

    # generates samples from a list of site names (can obtain using Predictive)
    def cget_all_samples_vec(self, dict_keys, rng_key, num_samples, svi_results = None, custom_mask = None):
        samples = {}
        for k in dict_keys:
            samples[k] = self.cget_samples_vec(k, rng_key, num_samples, svi_results, custom_mask)
        return samples
...
#             else:
#                 mu += S_nfw * self.nfw_temp_fixed[~mask]
